...
public class File16Main {
	...
	public static void main(String[] args) {
		...
		// 위 경로를 사용하여 File 객체 생성
		File f = new File(path);

		System.out.println();
		// 폴더 생성: mkdir()
		if(!f.exists()){  // 물리적으로 존재하는지 확인
			// 디렉토리가 없다면
			if(f.mkdir()){  // 디렉토리 생성
		...
		System.out.println();
		// 파일 생성 : createNewFile()
		File f2 = new File(f, TEST_FILE);   // '파일'을 표현하는 File 객체
		...
		File f3 = new File(f, TEST_RENAME);    // 변경될 파일 표현 객체
		System.out.println(f3.getAbsolutePath());

		if(f2.exists()){
			// 파일이 존재할때만 변경 (renameTo)
			if(f2.renameTo(f3)){
