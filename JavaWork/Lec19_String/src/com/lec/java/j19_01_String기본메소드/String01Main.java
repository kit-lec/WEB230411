...
public class String01Main {
	...
	public static void main(String[] args) {
		...
		str1.concat(str2);
		System.out.println(str1);  // <- str1은 변경되지 않는다

		// 원본 str1 을 변경하려면?
		str1 = str1.concat(str2);  // 변수 덮어쓰기 해야 한다!
		System.out.println(str1);

		// ★주의★
		// empty 문자열과 null 은 다르다
		// null 인 경우 메소드 수행하면 NullPointerException 발생!
		str3 = "";  // empty 문자열, 문자열 객체는 존재하나 비어있는 문자열
		System.out.println(str3.length());
		str3 = null; // null, 문자열 객체가 존재하지 않음
		//System.out.println(str3.length());  // null 다음에 . 붙이면 NullPointerException (NPE) 발생
